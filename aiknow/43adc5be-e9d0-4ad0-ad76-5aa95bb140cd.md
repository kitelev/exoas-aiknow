...
exo__Asset_updatedAt: 2026-09-21T10:19:03
exo__Instance_class:
...
|---|---|---|
| **§A1** | `2026-07-24` | Копируешь `exo__Asset_isDefinedBy` у ОДНОГО соседа того же класса ⇒ конвенция класса бывает РАСЩЕПЛЕНА (разные инстансы анкерятся по-разному), и зеркалирование наследует чужой выбор АУДИТОРИИ ⇒ перечислить ВСЕ инстансы с их анкерами (`SELECT ?a ?idb WHERE { ?a exo:Instance_class <class-IRI> ; exo:Asset_isDefinedBy ?idb }`); единообразно ⇒ копировать безопасно, расщеплено ⇒ выбирать по СЕМАНТИКЕ контента (личное → `$my-assets` · круг → `$shared-*` · работа → `exoas-tbank`). ⛔ Неверный audience-anchor = утечка приватности, а не косметика: ассет физически ляжет в чужой по аудитории репозиторий |
| **§A2** | `2026-08-02` | Создаёшь НОВУЮ онтологию с нуля и привязываешь к её анкеру ВСЁ, включая экземпляры ⇒ co-location кладёт ассет в папку его онтологии, а у ассетспейса есть АУДИТОРИЯ: один анкер на TBox и ABox уносит приватные экземпляры туда же, куда общедоступную модель ⇒ анкеры РАЗНЫЕ по построению: TBox (классы/свойства/перечисления) → общедоступное пространство (`exoas-public` — там весь shareable словарь; ⛔ анкер резолвить `find`-ом, не по памяти), ABox (экземпляры) → пространство, чья аудитория совпадает с аудиторией данных. ⚠ Tell ловится ОДНОЙ командой: `ls`, в какой ассетспейс ФАКТИЧЕСКИ легли экземпляры — не догадка. Пока не запушено, чинится сменой `isDefinedBy` + переносом, идентификаторы не меняются |
| **§A3** | `2026-08-12` | copy-from-neighbour у ЗРЕЛОГО ассета ⇒ вместе со свойствами копируется ПРОЗА ТЕЛА, а в ней утверждения о поведении движка («резолвер читает только первый класс», «priority бьёт безусловную спеку»), которые стареют вместе с кодом ⇒ N копий = N носителей одного ложного утверждения, выглядящих независимым подтверждением ⇒ техническое утверждение эталона проверять `git grep` по `origin/main` (⛔ не по локальному чекауту) ПЕРЕД копированием; опроверг — править и ЭТАЛОН, иначе следующий копирующий унаследует то же. ⚠ Tell: в теле фраза о том, КАК работает код (имя метода, «читает только», «побеждает»), а дата старше недель |
| **§A4** | `2026-08-16` | Dedup по русскому слову вернул НОЛЬ общих понятий — только частные случаи ⇒ каноническое имя концепта в этом vault часто АНГЛИЙСКОЕ, а рабочее русское живёт в `exo__Asset_aliases`, и поиск по `exo__Asset_label` его не видит ⇒ один запрос ОБОИМИ языками И по алиасам (`CONTAINS` по `?l` и по `?a`, КОРЕНЬ слова, не полная форма — склонения). ⛔ Цена — дубль поверх ЦЕНТРА таксономии (18 входящих у найденного постфактум), расщепляющий граф надвое; ⛤ корень промаха — bypass `/exocortex-asset` GATE 0, который ровно это и делает. ⚠ Tell ДО создания: тема заведомо общеупотребительна (роль, правило, игра), а dedup дал только частные случаи |
| **§A5** | `2026-08-24` | Заводишь тикет по инструкции «смежная находка → отдельный тикет» ⇒ у ОБЩЕГО инструмента (шаблон спавн-промпта, хук, CLI-обёртка, генератор) ты почти никогда не первый: дефект бьёт по КАЖДОМУ исполнителю одинаково на одном шаге, и дубли растут ЛИНЕЙНО по числу прогонов конвейера ⇒ один запрос по work-item'ам ДО `create` (⛔ фильтр по `ems__Effort_status` обязателен — иначе выдачу забивают `dec__Option` прошлых развилок), свип по ДИЗЪЮНКЦИИ 2-3 ключей из РАЗНЫХ слоёв (что сломано · чем · где). ⛔ Нашёл дубли — НЕ заводить «ещё один, зато точнее сформулированный»: это и есть следующий экземпляр класса |
| **§A6** | `2026-08-26` | Выделяешь имя из КОНЕЧНОГО множества в общем скрипте (код возврата, маркер журнала, env-переменная, позиция аргумента) и читаешь занятое прямо из файла ⇒ файл показывает ПРОШЛОЕ, а множество делят тикеты В ПОЛЁТЕ: сосед уже зарезервировал значение в своей спецификации и ещё не приземлился ⇒ ⛤ ФОРМА запроса — в §A5 (фильтр по `ems__Effort_status` обязателен, дизъюнкция 2-3 ключей): §A6 и §A5 — ОДИН механизм («спроси реестр в полёте, а не файл/память»), различаются реестром ⇒ перечислить тикеты доски, чей предмет — ТОТ ЖЕ файл, и прочитать их §Что сделать; тот же перечень отвечает и на ПОРЯДОК ПРИЗЕМЛЕНИЯ (кто меняет сигнатуру, которую правишь и ты). ⛔ Не «взять с запасом» — дыра в словаре исходов читается как утраченное значение |
