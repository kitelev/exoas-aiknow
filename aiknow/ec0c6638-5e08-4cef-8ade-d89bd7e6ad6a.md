...
| **§A6** | `2026-09-11` | Изолированная фикстура vault под CLI-смок: `create --class <label>` падает `❌ Class '<label>' not found in vault` на ВСЕХ входах разом ⇒ ⛔ дефект в ИЗМЕРИТЕЛЕ, не в предмете (§Второй признак `self-authored-claim-loses-its-provenance`: одинаковый ответ на заведомо различающихся входах). Механизм: `ClassResolverService.buildIndex` считает файл class-definition либо по legacy label-форме `[[exo__Class]]`, либо по UID метакласса, **обнаруженному в САМОМ vault** (файл с `exo__Asset_label: exo__Class`) ⇒ в фикстуре без метакласс-файла UID-форма не резолвится НИКОГДА, даже верная. ✅ Floor: форму фикстуры брать ЧТЕНИЕМ резолвера, а не по образцу боевого ассета — в изоляции работает label-форма. ⚠ Tell до прогона: копируешь в фикстуру UID, чей ТАРГЕТ в неё не копируется. ⛤ Разбора в справочнике НЕТ — floor целиком в этой строке _(2026-09-11, `3f8b640f`: 0/5 осей смока при исправном продукте; после смены формы — 5/5)_ |
